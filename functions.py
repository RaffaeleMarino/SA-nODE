...
class DatasetMaker:
    ...
    def attractor_shape_maker(self):
        ...
        for i in range(self.number_classes):
            ...
            attractor_list_y.append(attractor.copy())  # Add the attractor to the list for y component
    ...
    def attractors_maker(self, concatenated: bool = False, plot: bool = True):
        ...
        if self.config.get('FixedPoints') is None:
            ...
            for i, tmp in enumerate(attractor_list_x):
                tmp[tmp == 0] = -stable_point
                tmp[tmp == 1] = stable_point

            return np.array(attractor_list_x).T
    ...
    def load_MNIST_dataset(self, perturb=0.0, spectral=False):
        def add_noise(x_true, noise_factor):
            x=x_true.copy()
            x_f=[]
            for i in x:
                image = i
                total_pixel = image.size
                num_pixels_to_change = int(total_pixel * noise_factor)
                selected_indices = np.random.choice(total_pixel, num_pixels_to_change, replace=False)
                selected_coordinates = np.unravel_index(selected_indices, image.shape)
                new_values = np.random.uniform(0, 255, size=num_pixels_to_change).astype(np.uint8)
                for i in range(num_pixels_to_change):
                    image[selected_coordinates[0][i], selected_coordinates[1][i]] = new_values[i]
                x_f.append(image)
            x_f=np.asarray(x_f)
            return x_f
        net_size=784
        ...
        in_shape = 28 * 28
        (x_train, y_train), (x, y_test) = mnist.load_data()
        x_test = add_noise(x, perturb)
    ...
    def load_FASHIONMNIST_dataset(self, perturb=0.0, spectral=False):
        def add_noise(x_true, noise_factor):
            x=x_true.copy()
            x_f=[]
            for i in x:
